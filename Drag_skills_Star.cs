...
public class Drag_skills_Star : MonoBehaviour, IDragHandler, IEndDragHandler
     , IPointerDownHandler, IPointerUpHandler
{
    ...
    void Awake()
    {
        _pooler = FindObjectOfType<Star_Pool>();
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (this.transform.position.y >= 50)
        {
            if (Physics.Raycast(ray, out rayhit, 6))
            {
                ...
                _pooler.Spawning("Star", rayhit.point + Vector3.up * 0.5f, Quaternion.identity);
            }
        }
